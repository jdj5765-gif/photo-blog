...
아래 네 가지를 이만큼 맞추십시오.
① 줄바꿈 리듬 — 한 문장을 2~3줄로 끊어 내리고, 2~4줄 덩어리마다 빈 줄을 넣는 간격
② 어미 — ~더라구요 / ~잖아요 / ~답니다 / ~했는데요
③ 웃음 표기(ㅋㅋㅋ)와 문장 끝 이모지의 밀도
④ 말줄임표로 뜸 들이기, 따옴표로 그 자리 대화 옮기기
...
export function buildUserText(opts: GenerateOptions, imageCount: number): string {
  ...
  if (opts.intro?.trim()) {
    lines.push(
      `가게가 내세우는 점 (가게가 직접 쓴 소개글):\n${opts.intro.trim()}\n` +
        `이건 제가 확인한 사실이 아니라 가게 측 설명입니다. 그래서 쓸 때 규칙이 있습니다.\n` +
        `- 무엇을 강조할지, 어느 메뉴를 앞세울지 정하는 데 먼저 씁니다.\n` +
        `- 이 소개글에만 있고 사진이나 확인된 정보로 뒷받침되지 않는 내용은 ` +
        `반드시 전해 들은 말투로 씁니다: '~라고 하네요', '~라고 합니다', '~라고 적혀 있더라고요'.\n` +
        `  예) 원목 가구는 전부 빈티지 제품이라고 하네요.\n` +
        `  예) 도우는 48시간 저온 숙성한다고 하더라고요.\n` +
        `- 사진으로 직접 확인되는 것(분위기·비주얼)이나 제가 적어드린 맛은 ` +
        `제 경험이므로 평소 말투 그대로 단정해서 씁니다. 여기에 '~라고 하네요'를 붙이지 마세요.\n` +
        `- 전언 표현은 글 전체에서 3번을 넘기지 않습니다. 너무 많으면 남의 말만 옮긴 글이 됩니다.\n` +
        `금지: 소개글 문장을 그대로 옮기거나 어미만 바꿔 쓰기, ` +
        `제가 먹지 않은 메뉴의 맛을 칭찬하기, 광고 문구 같은 과장.`,
    );
  }
